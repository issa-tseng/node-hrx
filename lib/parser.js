      ...
      for (const char of decoder.write(chunk)) {
        ...
        // postboundary, we will immediately go to file|directory or comment state:
          ...
          if (char === ' ') {
            ...
            return done(new ParseProblem(`expected space or newline, got "${char}"`, position.line, position.col));
          }
        ...
        // if we should create a file or a directory based on the path:
          ...
          if ((char === ' ') && (path.length === 0)) {
            ...
            if (last(path) === '/') {
              ...
              return done(new ParseProblem('expected path, got newline', position.line, position.col));
            }
          } else if (!invalidPathChar(char)) {
            path += char;
          } else {
            return done(new ParseProblem(`illegal path character ${char}`, position.line, position.col));
          }
