...
const parseRecords = (inStream) => {
  ...
  const parserTransformer = new Transform({
      ...
      for (const char of decoder.write(chunk)) {
        ...
        // if we should create a file or a directory based on the path:
          ...
          if ((char === ' ') && (path.length === 0)) {
            ...
            if (last(path) === '/') { // TODO: last is probably terrible for perf
              startBody();
            ...
            // TODO: also a big perf hit probably.
            if (path.length === 0) {
              return done(new ParseProblem('expected path component, got /', position));
            } else if (last(path) === '/') {
              return done(new ParseProblem('illegal empty path component', position));
            } else if (path.endsWith('/.')) {
              return done(new ParseProblem('illegal path component .', position));
            } else if (path.endsWith('/..')) {
              return done(new ParseProblem('illegal path component ..', position));
            } else {
              path += char;
    ...
    flush(done) {
      // failure cases at end-of-file:
      if (state === State.init)
        return done(new ParseProblem('expected initial boundary, got end of file', position));

      if (state === State.postBoundary)
        return done(new ParseProblem('expected newline or space, got end of file', position));

      if (state === State.path)
        return done(new ParseProblem('expected path, got end of file', position));

      // success path at end-of-file; flush in-progress record.
